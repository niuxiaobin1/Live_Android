...
public class VideoScrollViewHolder extends AbsViewHolder implements
    ...
    public VideoScrollViewHolder(Fragment fragment, ViewGroup parentView, int position, String videoKey, int page) {
        super(fragment, parentView, position, videoKey, page);
        mFragment=fragment;
        init();
    ...
    public void init() {
        ...
        if (mFragment!=null){
            mVideoScrollAdapter = new VideoScrollAdapter(mFragment, list, mPosition);
        }else{
    ...
    private void openCommentInputWindow(boolean openFace) {
        if (mVideoBean != null) {
            if (mContext instanceof AbsVideoPlayActivity){
                ...
                ((VideoPlayFragment)mFragment).openCommentInputWindow(openFace, mVideoBean.getId(), mVideoBean.getUid(), null);
            }
        }
    }
