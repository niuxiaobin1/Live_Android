...
public class VideoScrollAdapter extends RecyclerView.Adapter<VideoScrollAdapter.Vh> implements ItemSlideHelper.Callback {
    ...
    public VideoScrollAdapter(Fragment fragment, List<VideoBean> list, int curPosition) {
        mFragment = fragment;
        ...
        mLikeAnimDrawables = new Drawable[likeImageList.size()];
        for (int i = 0, length = mLikeAnimDrawables.length; i < length; i++) {
            mLikeAnimDrawables[i] = ContextCompat.getDrawable(mContext, likeImageList.get(i));
        }
    }
    ...
    public Vh onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        VideoPlayWrapViewHolder vpvh;
        if (mFragment != null) {
            vpvh = new VideoPlayWrapViewHolder(mFragment, parent);
        } else {
            vpvh = new VideoPlayWrapViewHolder(mContext, parent);
        }
    ...
    public void onLeftScroll(RecyclerView.ViewHolder vh) {
        if (mFragment!=null){
            if (((VideoPlayFragment) mFragment).isPaused()) {
                return;
            }
        }else{
